...
class OpenhabConnection {
    ...
    async _handleEventSourceError(error) {
        ...
        this.state = STATE.DOWN;  // set intermediate state, so _enterConnecting may schedule reconnect
    ...
    _scheduleReconnect() {
        ...
        // avoid retry storms by adding a random jitter of up to 30% of the current retry delay. 
        // This is especially important if multiple nodes are configured with the same OpenHAB URL, 
        // as they would otherwise all retry at the same time and cause unnecessary load on the system.
        const jitter = Math.random() * this.currentRetryDelay * 0.3;
